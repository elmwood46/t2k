...
public partial class Coin : RigidBody3D
{
    ...
    {        
        ...
        {
            ((MeshInstance3D)GetChild(0)).Scale = Vector3.One*(float)Mathf.Clamp(_deathtimer.TimeLeft/_deathtimer.WaitTime,0.1f,1.0f);
            ((CollisionShape3D)GetChild(1)).Scale = Vector3.One*(float)Mathf.Clamp(_deathtimer.TimeLeft/_deathtimer.WaitTime,0.1f,1.0f);
        }
